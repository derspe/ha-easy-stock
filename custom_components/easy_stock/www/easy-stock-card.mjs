...
let EasyStockCard = class extends i {
  ...
  setConfig(config) {
    if (!config.entity) throw new Error("easy-stock-card: 'entity' is required.");
    this._config = { show_chart: true, ...config };
  }
  getCardSize() {
    var _a2;
    return ((_a2 = this._config) == null ? void 0 : _a2.show_chart) ? 4 : 2;
  }
  static getStubConfig() {
    return { type: "custom:easy-stock-card", entity: "sensor.stock_sap_de", show_chart: true };
  }
  _sparkline(history) {
    if (!history || history.length < 2) return A;
    const W = 500;
    const H2 = 80;
    const min = Math.min(...history);
    const max = Math.max(...history);
    const range = max - min || 1;
    const pts = history.map((v2, i2) => {
      const x2 = i2 / (history.length - 1) * W;
      const y3 = H2 - (v2 - min) / range * H2;
      return `${x2.toFixed(1)},${y3.toFixed(1)}`;
    });
    const first = history[0];
    const last = history[history.length - 1];
    const positive = last >= first;
    const color = positive ? "var(--success-color, #4caf50)" : "var(--error-color, #f44336)";
    const fillPts = `0,${H2} ${pts.join(" ")} ${W},${H2}`;
    return w`
      <svg viewBox="0 0 ${W} ${H2}" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
  ...
  render() {
    ...
    const price = parseFloat(this._entity.state);
    const attr = this._entity.attributes;
    const positive = attr.change_pct >= 0;
    const changeClass = positive ? "positive" : "negative";
    const changeSign = positive ? "+" : "";
    const title = this._config.title ?? attr.long_name ?? attr.symbol;
    return b`
      <ha-card>
        <div class="container">
          <div class="header">
            <div class="name">${title}</div>
            <div class="symbol">${attr.symbol}</div>
          </div>

          <div class="price-row">
            <div class="price">
              ${isNaN(price) ? "–" : price.toLocaleString(void 0, { minimumFractionDigits: 2, maximumFractionDigits: 4 })}
              <span class="currency">${attr.currency}</span>
            </div>
            <div class="change ${changeClass}">
              <span>${changeSign}${(_a2 = attr.change) == null ? void 0 : _a2.toFixed(2)}</span>
              <span class="pct">(${changeSign}${(_b = attr.change_pct) == null ? void 0 : _b.toFixed(2)}%)</span>
            </div>
          </div>

          ${this._config.show_chart && ((_c = attr.history) == null ? void 0 : _c.length) ? b`<div class="chart">${this._sparkline(attr.history)}</div>` : A}
        </div>
      </ha-card>
    `;
  }
...
__decorateClass([
  r()
], EasyStockCard.prototype, "_entity", 2);
__decorateClass([
  r()
...
window.customCards.push({
  ...
  description: "Displays a stock price with 1-year sparkline chart from Yahoo Finance."
});
export {
  EasyStockCard
};
//# sourceMappingURL=easy-stock-card.mjs.map
